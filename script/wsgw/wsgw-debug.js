...
(async () => {
  if ((await showNotice(), !USERNAME || !PASSWORD))
    return done({response:{status:200,headers:{"content-type":"text/plain;charset=utf-8"},body:"WSGW_DEBUG NO_CREDS"}}), sendMsg(
      SCRIPTNAME,
      "请先配置网上国网账号密码!",
      "点击前往BoxJs配置",
      {
  ...
  .catch((e) => {
    /无效|失效|过期|重新获取|请求异常/.test(e) &&
      ...
      log.error(e);
    done({response:{status:200,headers:{"content-type":"text/plain;charset=utf-8"},body:"WSGW_DEBUG CATCH " + String((e && (e.stack || e.message)) || e)}});
  })
  .finally(() => done({response:{status:200,headers:{"content-type":"text/plain;charset=utf-8"},body:"WSGW_DEBUG FINISHED_NO_PAYLOAD"}}));
